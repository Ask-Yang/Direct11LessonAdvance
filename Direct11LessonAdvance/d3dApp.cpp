...
D3DApp::D3DApp(HINSTANCE hInstance)
	...
	m_MainWndCaption(L"DX11 Lesson Advance"),
	m_ClientWidth(800),
	m_ClientHeight(600),
	m_hMainWnd(nullptr),
	m_AppPaused(false),
...
{
	CheckResouceAndRelease();
	ResetSwapChainAndBuildNewTargetView();
	AppBuildDepthStencil();
	m_pd3dImmediateContext->OMSetRenderTargets(1, m_pRenderTargetView.GetAddressOf(), m_pDepthStencilView.Get());
	
	SetAppViewPortChange();
...
{
	HRESULT hr = S_OK;
	if (!AppCreateD3D11Device(hr))
		return false;

	if (!AppCreateD3D11SwapChain(hr))
		return false;
...
void D3DApp::CheckResouceAndRelease()
{
...
void D3DApp::ResetSwapChainAndBuildNewTargetView()
{
	// 重设交换链并且重新创建渲染目标视图
	ComPtr<ID3D11Texture2D> backBuffer;
	HR(m_pSwapChain->ResizeBuffers(1, m_ClientWidth, m_ClientHeight, AppDxgiFormat, 0));
...
void D3DApp::AppBuildDepthStencil()
{
...
void D3DApp::SetAppViewPortChange()
{
	// 设置视口变换
...
void D3DApp::InitMouseAndKeyboard()
{
	m_pMouse = std::make_unique<DirectX::Mouse>();
	m_pKeyboard = std::make_unique<DirectX::Keyboard>();
}

bool D3DApp::BuildWndClass(WNDCLASS& wc)
{
...
bool D3DApp::AdjustWindow(int& width, int& height)
{
...
bool D3DApp::AppCreateWindow(int width, int height)
{
...
bool D3DApp::AppCreateD3D11Device(HRESULT& hr)
{
	// 创建D3D设备 和 D3D设备上下文
	...
	return D3D11DeviceCheck(featureLevel);
}

bool D3DApp::D3D11DeviceCheck(D3D_FEATURE_LEVEL& featureLevel)
{
	// 检测是否支持特性等级11.0或11.1
	...
	m_pd3dDevice->CheckMultisampleQualityLevels(
		AppDxgiFormat, 4, &m_4xMsaaQuality);
	assert(m_4xMsaaQuality > 0);
	return true;
}

bool D3DApp::AppCreateD3D11SwapChain(HRESULT& hr)
{
	...
	{
		return AppCreateD3D11_1SwapChain(dxgiFactory2);
	}
	else
	{
		return AppCreateD3D11_0SwapChain(dxgiFactory1);
	}
}

bool D3DApp::AppCreateD3D11_1SwapChain(ComPtr<IDXGIFactory2> dxgiFactory2)
{
...
bool D3DApp::AppCreateD3D11_0SwapChain(ComPtr<IDXGIFactory1> dxgiFactory1)
{
	...
	sd.BufferDesc.Format = AppDxgiFormat;
...
bool D3DApp::InitMainWindow()
{
	...
	if (!BuildWndClass(wc))
		return false;
	int width, height;
	if (!AdjustWindow(width, height))
		return false;
	if (!AppCreateWindow(width, height))
		return false;
...
Graphic::Graphic(ComPtr<ID3D11DeviceContext> pd3dImmediateContext, ComPtr<ID3D11Device> pd3dDevice)
{
...
{
	m_pd3dDevice = g.m_pd3dDevice;
	m_pd3dImmediateContext = g.m_pd3dImmediateContext;
}
...
ComPtr<ID3D11DeviceContext> Graphic::GetContext()
{
